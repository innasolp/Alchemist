...
public class GetAllCategoryChildrenRequestHandler(AlchemyContext context) : IRequestHandler<GetAllCategoryChildrenRequest, List<ShopCategory>>
{
    private readonly SemaphoreSlim _addCategoryChildrenSemaphore = new(1, 1);
    private AlchemyContext Context { get; } = context;

    public async Task<List<ShopCategory>> Handle(GetAllCategoryChildrenRequest request, CancellationToken cancellationToken = default)
    {
