...
internal static class HeadersHelper
{
    ...
    public static Dictionary<string, string> GetHeadersForRequest(RequestHeaders requestHeaders, IEnumerable<ICookieData> cookieValues)
    {        
        var headers = new Dictionary<string, string>(requestHeaders.Headers);       

        var cookies = GetCookieValues(requestHeaders.CookieKeys, cookieValues, 0);
